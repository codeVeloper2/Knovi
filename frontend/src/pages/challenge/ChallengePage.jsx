...
function WaitingRoom({ challenge, busy, onPrepare, onReady, onRefresh, onBack }) {
  ...
  return <div className="challenge-page challenge-detail-page"><ChallengeHeader onBack={onBack} title="Challenge Room" subtitle={aiMode ? "UPRAD is your challenge partner." : "Waiting for both players to be ready…"} /><div className="challenge-room card"><div className="challenge-versus">
    <div className="challenge-player"><Avatar size={68} name="You" /><strong>You</strong><span className={ready ? "ready" : "not-ready"}>{ready ? "Ready" : "Not ready"}</span></div>
    <div className="challenge-vs">{aiMode ? "VS" : "VS"}</div>
    <div className="challenge-player">{aiMode ? <><Avatar size={68} name="UPRAD" /><strong>UPRAD</strong><span className="ready">AI Challenge</span></> : <><Avatar size={68} name={challenge.opponent?.displayName} url={challenge.opponent?.photoURL} online={challenge.opponent?.isOnline} /><strong>{challenge.opponent?.displayName}</strong><span className={challenge.opponentReady ? "ready" : "not-ready"}>{challenge.opponentReady ? "Ready" : "Getting ready"}</span></>}</div>
  </div><div className="challenge-room-summary"><span>{challenge.subjectName}</span><span>·</span><span>{challenge.topicName}</span><span>·</span><span>{challenge.conceptName}</span><span>·</span><span>{challenge.questionCount} questions</span></div><div className="challenge-progress-steps"><span className="done">Challenge created</span><span className={preparing ? "current" : challenge.status === "waiting" ? "done" : ""}>Prepare questions</span><span className={challenge.status === "waiting" && !ready ? "current" : "done"}>{aiMode ? "UPRAD ready" : "Ready up"}</span><span>Countdown</span></div><p className="challenge-waiting-reason">{aiMode ? (preparing ? "UPRAD is preparing questions from what you learned." : challenge.waitingReason) : challenge.waitingReason}</p><div className="challenge-room-actions">{challenge.status === "accepted" && <button type="button" className="btn btn-primary" onClick={onPrepare} disabled={busy}>{busy ? "Preparing…" : "Prepare Battle"}</button>}{challenge.status === "preparing" && <div className="challenge-preparing"><span className="challenge-spinner" /> AI questions are being prepared from the shared learning context.</div>}{challenge.status === "waiting" && <button type="button" className="btn btn-primary" onClick={onReady} disabled={busy || ready}>{ready ? (aiMode ? "UPRAD is ready" : "You're ready") : busy ? "Saving…" : "I'm Ready"}</button>}<button type="button" className="btn btn-ghost" onClick={onRefresh}>Refresh</button></div></div></div>;
}
